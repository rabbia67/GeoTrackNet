...
def elbo(cell,
    ...
    data_dim = tf.shape(inputs[0])[-1]  # This line is not used but kept for consistency
    ...
    init_inputs, init_mask = nested.read_tas([inputs_ta, mask_ta], t0)  # This line is not used but kept for consistency

    ta_names = ['log_weights', 'log_ess']
    tas = [tf.TensorArray(tf.float32, max_seq_len, name='%s_ta' % n)
           for n in ta_names]

    # FIX: Ensure that the shape argument for tf.zeros is constructed using tf.stack
    # to avoid the NotImplementedError when dealing with symbolic tensors.
...
def fivo(cell,
    ...
    # FIX: Apply the same tf.stack fix for tf.zeros in fivo as well.
    tf_num_samples = tf.convert_to_tensor(num_samples, dtype=tf.int32)
    tf_batch_size = tf.convert_to_tensor(batch_size, dtype=tf.int32)
