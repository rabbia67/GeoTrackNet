...
class VRNNCell(snt.AbstractModule):
    ...
    def zero_state(self, batch_size, dtype):
        ...
        """
        # Fix for NotImplementedError: Cannot convert a symbolic Tensor to a numpy array.
        # Ensure that `batch_size` is treated as a TensorFlow tensor when constructing
        # the shape for `tf.zeros`, preventing `np.prod` from being called on a symbolic tensor.
        tf_batch_size = tf.convert_to_tensor(batch_size, dtype=tf.int32)

        return (self.rnn_cell.zero_state(batch_size, dtype),
                tf.zeros(tf.stack([tf_batch_size, self.encoded_z_size]), dtype=dtype))
    ...
    def _build(self, observations, state, mask, return_value=None):
        ...
        rnn_out, new_rnn_state = self.rnn_cell(rnn_inputs,
                                               rnn_state)  # STEP 3: o_t, h_t = RNN(phi_x(x_t-1), phi_z(z_t-1), h_t-1)
        ...
        latent_state_prior = latent_dist_prior.sample(
            seed=self.random_seed)  # z_t ~ q(z_t|x_t) = phi_enc(o_t, phi_x(x_t))
        ...
        generative_dist = self.generative(latent_encoded,
                                          rnn_out)  # STEP 2: p(x_t|z_t) = phi_dec(z_t, o_t) = p(x_t|z_<=t, x<t)
        log_p_x_given_z = tf.reduce_sum(generative_dist.log_prob(targets), axis=-1)

        generative_dist_prior = self.generative(latent_prior_encoded,
                                                rnn_out)  # STEP 2: p(x_t|z_t) = phi_dec(z_t, o_t) = p(x_t|z_<=t, x<t)
    ...
    def sample(self, observations, state):
        with tf.variable_scope("vrnn"):
            ...
            # and approximate posterior q.
            # Create the generative dist. and calculate the logprob of the targets.
            generative_dist = self.generative(latent_prior_encoded,
                                              rnn_out)  # STEP 2: p(x_t|z_t) = phi_dec(z_t, o_t) = p(x_t|z_<=t, x<t)
